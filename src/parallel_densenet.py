...
class SimpleView(nn.Module):
    def __init__(self, n_classes: int, n_views: int):
        ...
        sides.features[0].weight = torch.nn.Parameter(
            sides.features[0].weight.sum(dim=1, keepdim=True)
        )
        ...
        tops.features[0].weight = torch.nn.Parameter(
            tops.features[0].weight.sum(dim=1, keepdim=True)
        )
        ...
        details.features[0].weight = torch.nn.Parameter(
            details.features[0].weight.sum(dim=1, keepdim=True)
        )
        ...
        self.height_pathway = nn.Sequential(
            nn.Linear(1, 128), nn.ReLU(), nn.Linear(128, z_dim), nn.ReLU()
        )
...
class TrainDataset_AllChannels:
    ...
    def __init__(
        ...
        self.max_points_per_tree = (
            max_points_per_tree  # None or int, to limit memory use in laspy mode
        )
        ...
        if isinstance(csv_or_las, str):
            ...
            valid_mask = (
                tree_ids_arr > 0
            )  # Exclude 0 and negative IDs - -1 is ground/unsegmented
            df_tmp = pd.DataFrame(
                {
            ...
            uniq, starts, counts = np.unique(
                self._tree_ids_np, return_index=True, return_counts=True
            )
            self._tid2span = {
                int(u): (int(s), int(s + c))
            ...
            raise TypeError(
                "csv_or_las must be a CSV file path or laspy.LasData object."
            )
    ...
    def _augment_torch(
        ...
        rx = (
            torch.rand((), device=pts.device) * 2 * rotate_h_max - rotate_h_max
        ) * deg2rad
        ry = (
            torch.rand((), device=pts.device) * 2 * rotate_h_max - rotate_h_max
        ) * deg2rad
        rz = (
            torch.rand((), device=pts.device) * 2 * rotate_v_max - rotate_v_max
        ) * deg2rad
    ...
    def __getitem__(self, idx):
        ...
        if hasattr(self, "las_data"):
            ...
            tree = np.column_stack(
                (self._x_np[lo:hi], self._y_np[lo:hi], self._z_np[lo:hi])
            )  # float32, shape [K,3]
            ...
            if self.pc_rotate:
                if self.projection_backend == "numpy":
                    image = sv.points_to_images(
                        au.augment(tree), res_im=self.res, num_side=self.n_sides
                    )
                    image = torch.from_numpy(image)
                elif self.projection_backend == "torch":  # torch
                    points = torch.from_numpy(tree).float()
                    points = self._augment_torch(points)
                    image = svt.points_to_images(
                        points, res_im=self.res, num_side=self.n_sides
                    )
                    # image = image.numpy()
                else:
                    raise ValueError(
                        f"Unknown projection_backend: {self.projection_backend}"
                    )
            else:
                if self.projection_backend == "numpy":
                    image = sv.points_to_images(
                        tree, res_im=self.res, num_side=self.n_sides
                    )
                    image = torch.from_numpy(image)
                elif self.projection_backend == "torch":  # torch
                    points = torch.from_numpy(tree).float()
            ...
            las_name = os.path.join(
                self.root_dir, *self.trees_frame.iloc[idx, 0].split("/")
            )
            if self.pc_rotate:
                if self.projection_backend == "numpy":
                    image = sv.points_to_images(
                        au.augment(las_name), res_im=self.res, num_side=self.n_sides
                    )
                elif self.projection_backend == "torch":  # torch
                ...
                if self.projection_backend == "numpy":
                    image = sv.points_to_images(
                        rl.read_las(las_name), res_im=self.res, num_side=self.n_sides
                    )
                elif self.projection_backend == "torch":  # torch
